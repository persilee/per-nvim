...
| **nvim-lspconfig** | 内置 10 个 LSP：`clangd`（C/C++）、`cmake`、`pyright`（自动探测 `.venv/venv/env` 虚拟环境）、`lua_ls`（识别 `vim`/`hs` 全局，加载 config 与 lazy 库）、`bashls`、`html`、`cssls`、`tsserver`（JS/TS）、`tex`（texlab）、`jsonls`（内置 package.json/tsconfig/.eslintrc 的 JSON Schema）、`marksman`（Markdown） |
...
| **flash.nvim**    | 超快字符跳转：`s` 输入字符实时过滤候选并打标签，支持 Treesitter 节点（`S`）、跨窗口远程跳转（`r`）；search 搜索模式已禁用（不接管 `/` 搜索），可替代原生 f/t 与 Hop |
...
| 插件                     | 功能要点                                                                                                                                                                                                                                                                                                             |
| ------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **render-markdown.nvim** | Markdown 实时渲染：标题图标（6 级）、代码块边框 + 语言图标标识、表格圆角边框、任务列表图标（含 `[?]` `[>]` `[-]` `[!]` `[~]` 自定义态）、Obsidian/GitHub 风格 callout（Abstract / Warning / Danger / Bug / TODO / WIP 等）、wiki / 图片 / github / gitlab / youtube 链接图标、blink + LSP 补全集成、插入模式同步渲染 |
| **bullets.nvim**         | 列表自动续写：markdown / 纯文本 / gitcommit 中按回车自动生成下一个列表项                                                                                                                                                                                                                                             |
| **img-clip.nvim**        | 剪贴板图片粘贴：markdown 存 `./attachments`（avif 压缩）、tex/typst 存 `./figs`，自动插入对应语法模板（markdown 为 `![image]($FILE_PATH)`，tex 为 figure 环境，typst 为 `#image`）                                                                                                                                   |
